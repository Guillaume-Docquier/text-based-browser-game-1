...
export function PlayersPage(): ReactElement {
  ...
  return (
    <section className="min-w-0 flex-1 px-4 py-5 sm:px-6 lg:px-8">
      ...
      <UpdateReadinessAlert error={updateReadiness.error} />
    </section>
  )
}
...
function PlayersTable({ children }: { children: ReactElement[] }): ReactElement {
  return (
    <Card>
  ...
  return (
    <div className="flex items-center gap-3 rounded-3xl border border-border/60 bg-muted/20 px-4 py-3">
      <PlayerReadiness
        ...
        disabled={playerView.turnStatus !== "COLLECTING_ACTIONS" || isReadinessPending}
        onToggle={() => {
          onUpdateReadiness(!playerState.isReady)
        }}
      />
      <span
...
function PlayerReadiness({
  ...
  if (isCurrentPlayer) {
    return (
      <Button variant="ghost" size="icon" aria-label="Ready" aria-pressed={isReady} disabled={disabled} onClick={onToggle}>
...
function UpdateReadinessAlert({ error }: { error: { message: string } | null }): ReactElement | null {
  if (error === null) {
    return null
  }
