      ...
      <Card className="border border-border/60">
        ...
        <CardContent className="grid gap-6 md:grid-cols-2">
          <div className="space-y-2 md:col-span-2">
            ...
            <Input id="game-name" value={name} onChange={(event) => setName(event.target.value)} placeholder="Galactic trade league" />
          </div>
          <div className="space-y-2">
            <Label htmlFor="nb-seats">Max number of players</Label>
            <Input id="nb-seats" type="number" value={nbSeats} onChange={(event) => setNbSeats(parseInt(event.target.value))} />
          </div>
          <div className="space-y-2">
            <Label>Turn length</Label>
            <div className="flex gap-3">
              <Input type="number" value={tickIntervalMultiplier} onChange={(event) => setTickIntervalMultiplier(parseInt(event.target.value))} />
              <Select value={tickIntervalUnit} onValueChange={(value) => setTickIntervalUnit(value as TickIntervalUnit)}>
          ...
          <div className="md:col-span-2">
            <Button
              ...
              onClick={() => {
                createGame.mutate({ newGame: { name, nbSeats, tickIntervalSeconds: Temporal.Duration.from({ [tickIntervalUnit]: tickIntervalMultiplier }).total("seconds") } }, { onSuccess: ({ newGame }) => void navigate({ to: "/games/$gameId", params: { gameId: newGame.id } }) })
              }}
            >
