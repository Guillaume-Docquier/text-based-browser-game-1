...
test("the actions view shows action costs and updates them when actions are selected", async ({ alice }) => {
  ...
  await test.step("Display Action descriptions and unaffordable costs", async () => {
  ...
  await test.step("Select multiple affordable Actions and restore their resources", async () => {
    const extractMetal = actionsPage.action("Extract Metal")
