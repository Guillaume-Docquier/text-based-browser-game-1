...
import { z } from "zod"
import { AccountsRepository, type AccountModel, type NewAccountModel } from "#lib/db/accounts/accounts.repository.ts"
...
async function seedAccounts({
  ...
  for (const newAccount of newAccounts) {
    accounts.push(assertSuccess(await accountsRepository.create(newAccount)))
  }
...
async function seedGames({
  ...
  assertSuccess(await gamesController.join({ gameId: insanelyFastGame.id, accountId: secondAccount.id }))
  assertSuccess(await gamesController.join({ gameId: insanelyFastGame.id, accountId: thirdAccount.id }))
  logger.info("└ Done")
}
