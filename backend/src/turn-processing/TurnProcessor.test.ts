...
describe("TurnProcessor", () => {
  ...
  describe("processNextDueTurn", () => {
    it("should process the current turn and queue the next one", async () => {
      ...
      // Act
      clock.increment({ time: turnInterval })
      await markDueTurnsAwaitingProcessing(turnsRepository, clock.now())
    ...
    it("should process a turn early when all players are ready and reset their readiness", async () => {
      ...
      const processingOutcome = await turnProcessor.processNextDueTurn()
...
async function markDueTurnsAwaitingProcessing(turnsRepository: TurnsRepository, since: Date): Promise<void> {
  Assert.isSuccess(await turnsRepository.markDueTurnsAwaitingProcessing({ since }))
}
