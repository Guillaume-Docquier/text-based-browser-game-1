import { CreatedGame, CreateGameCommand, type GamesController, GameSummary } from "./games.controller.ts"
...
export function createGamesRouter({ trpc, gamesController, ...others }: { trpc: Trpc; gamesController: GamesController; logger: Logger }) {
  ...
  return trpc.router({
    ...
    create: trpc.privateProcedure
      .input(z.object({ newGame: CreateGameCommand.omit({ createdByPlayerId: true }) }))
