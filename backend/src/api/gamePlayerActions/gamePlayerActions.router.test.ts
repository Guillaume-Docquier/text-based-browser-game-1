...
describe("game player actions router", () => {
  it("sets and gets the current action for the authenticated player", async () => {
    // Arrange
  ...
  it("rejects setting an action for a stale tick", async () => {
    // Arrange
    const db = await createDbMock()
    const player = await createPlayer(db, createPlayerRowInsertStub())
    ...
    await expect(
      ...
      message: "Cannot submit action for tick 1, the game is currently at tick 0.",
    })
  })

  it("rejects anonymous action reads", async () => {
    // Arrange
