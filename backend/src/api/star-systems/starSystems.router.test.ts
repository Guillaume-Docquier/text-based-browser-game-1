...
describe("starSystems.router", () => {
  describe("getSystem", () => {
    it("should get the full stored system for an authenticated account in the game", async () => {
      // Arrange
    ...
    it("should reject an account that does not have a player in the game", async () => {
      // Arrange
      const { api, authService, accountsRepository, starSystemsRepository } = await createApiStub()
      using trpcClient = new TrpcClient({ api })

      const creator = extractSuccess(await accountsRepository.create(createNewAccountModelStub({ alias: "Creator" })))
      const outsider = extractSuccess(await accountsRepository.create(createNewAccountModelStub({ alias: "Outsider" })))
