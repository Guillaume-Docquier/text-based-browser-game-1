...
describe("rng", () => {
  describe("float", () => {
    ...
    it.each([
      ...
      { value: 0, expected: 10, min: 10, maxInclusive: 20 },
      { value: 0.35, expected: 13.5, min: 10, maxInclusive: 20 },
      { value: 0.5, expected: 15, min: 10, maxInclusive: 20 },
      { value: 0.65, expected: 16.5, min: 10, maxInclusive: 20 },
      { value: 1 - Number.EPSILON, expected: 20, min: 10, maxInclusive: 20 }, // An example where float range is inclusive because of floating point errors
      // Floats smaller than 1
      { value: 0, expected: 0.3, min: 0.3, maxInclusive: 0.7 },
      { value: 0.35, expected: 0.43999999999999995, min: 0.3, maxInclusive: 0.7 },
      { value: 0.5, expected: 0.5, min: 0.3, maxInclusive: 0.7 },
      { value: 0.65, expected: 0.56, min: 0.3, maxInclusive: 0.7 },
      { value: 1 - Number.EPSILON, expected: 0.6999999999999998, min: 0.3, maxInclusive: 0.7 },
    ])("should return values in the range when min and max are different", ({ value, expected, min, maxInclusive }) => {
      // Arrange
      const range = Range.createMaxInclusive({ numericType: "float", min, maxInclusive })
      const rng = createRng(() => value)
    ...
    it.each([0, 0.25, 0.5, 0.75, 1 - Number.EPSILON])(
      ...
      (value) => {
        // Arrange
        const range = Range.createMaxInclusive({ numericType: "float", min: 10, maxInclusive: 10 })
        const rng = createRng(() => value)
    ...
    it.each([0, 0.25, 0.5, 0.75, 1 - Number.EPSILON])("should return exactly 1 when min and max are 1", (value) => {
      // Arrange
      const range = Range.createMaxInclusive({ numericType: "float", min: 1, maxInclusive: 1 })
      const rng = createRng(() => value)
  ...
  describe("int", () => {
    it.each([
      ...
      // Arrange
      const range = Range.createMaxInclusive({ numericType: "integer", min: 10, maxInclusive: 20 })
      const rng = createRng(() => value)
    ...
    it.each([0, 0.25, 0.5, 0.75, 1 - Number.EPSILON])(
      ...
      (value) => {
        // Arrange
        const range = Range.createMaxInclusive({ numericType: "integer", min: 10, maxInclusive: 10 })
        const rng = createRng(() => value)
