...
export const EffectFactory = {
  ...
  fromActionSubmission: (actionSubmission: ActionSubmission, ruleset: Ruleset): Effect[] => {
    ...
    return mechanics.map((mechanic) => EffectFactory.fromMechanic({ mechanic, targets }))
  },
  /**
   * Creates an effect for a mechanic
   */
  fromMechanic: ({ mechanic, targets }: { mechanic: Mechanic; targets: ActionSubmission["targets"] }): Effect => {
    switch (mechanic.type) {
      case ResourceLossMechanic.type:
        return new ResourceLossEffect(mechanic, targets)
      case ResourceGainMechanic.type:
        return new ResourceGainEffect(mechanic, targets)
      case VictoryMechanic.type:
        return new VictoryEffect(mechanic, targets)
    }
  },
}
