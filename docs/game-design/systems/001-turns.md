...
The Fleet Build Phase creates or merges stationary Fleets at their targeted Planets before Combat. After the Movement and Fleet Build Phases, Combat resolves from the final Fleet positions. The Planet Phase occurs before Colonization. Colonization then uses the arrival order established by Movement Ticks. A Fleet that arrives during the Turn is present for Combat at its destination before it can colonize that Planet.
