...
| Home Planet               | The Planet assigned to a Player when a game starts.                                                                                                                                                |
...
| Target Type               | The base entity kind selected by an Action target slot: `FLEET`, `PLANET`, or `PLAYER`. Eligibility such as ownership is expressed separately by Target Constraints.                               |
| Target Constraint         | A composable rule that determines whether a resolved Action target is eligible. Every effective constraint must be satisfied.                                                                      |
| Resolved Target           | A selected target id resolved to a base Target Type and its Fleet, Planet, or Player entity before Target Constraints are evaluated.                                                               |
...
| Readiness                 | A public, reversible player state that locks their Actions. When all players are Ready, the Turn ends early.                                                                                       |
...
| Tick                      | An internal precision sub-step within the Fleet Movement Phase used to sequence movement progress and arrivals.                                                                                    |
