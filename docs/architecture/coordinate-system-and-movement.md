...
1. [World Map Persistence And Read API](#Phase-1-World-Map-Persistence-And-Read-API)
2. [Game Layout And Actions View](#Phase-2-Game-Layout-And-Actions-View)
3. [Game Creation Settings And Deterministic World Generation](#Phase-3-Game-Creation-Settings-And-Deterministic-World-Generation)
4. [Star Map View](#Phase-4-Star-Map-View)
...
### Phase 1: World Map Persistence And Read API

Goal: add the static world map schema, movement graph persistence, repository reads, player-authorized controller queries, and a tRPC router. This phase does not generate maps during game creation; it makes the storage and read boundary available.
...
- `backend/src/lib/db/worldMaps.repository.ts`
- `backend/src/api/world-maps/worldMaps.controller.ts`
- `backend/src/api/world-maps/worldMaps.router.ts`
- `backend/src/api/world-maps/worldMaps.router.test.ts`
...
1. Add the map tables to `backend/src/lib/db/schema.ts`: `game_maps`, `game_map_movement_nodes`, `game_map_orbits`, `game_map_sectors`, `game_map_bodies` and `game_map_movement_edges`.
2. Match the constraints from the Architecture section exactly: cascade from `games` to `game_maps`, composite same-game foreign keys for child rows, directed movement edges with primary key `(game_id, from_node_id, to_node_id)`, unique movement node references from concrete targets, and indexes on game/parent lookup columns.
...
6. Follow the table definition for `game_map_movement_edges`: do not add an undocumented edge id just because the illustrative `MovementEdge` type above contains one. Return `{ from, to, weight }` from API DTOs.
7. Generate the migration with `pnpm --filter backend db:generate --name world-maps`.
8. Implement `WorldMapsRepository` with public methods `createSystem`, `getSystem`, `getSector`, `getBody` and `areNeighbors`.
...
- `worldMaps.getSystem` returns the full stored System for an authenticated player in the game.
- `worldMaps.getSystem` rejects anonymous reads with `UNAUTHORIZED`.
...
Goal: reshape the play surface into a game layout with a left navigation bar, top game info bar, Star Map route, and Actions route. This phase keeps the current action behavior but moves it into the Actions page.

Files to create or update:

- `frontend/src/routes/play.$gameId.tsx`
- `frontend/src/routes/play.$gameId.index.tsx`
- `frontend/src/routes/play.$gameId.map.tsx`
...
5. Add `GameSideNav` with two links: Star Map at `/play/$gameId/map` and Actions at `/play/$gameId/actions`.
...
7. Add `play.$gameId.index.tsx` so `/play/$gameId` lands on the Star Map experience. During Phase 2 it may render the same temporary map placeholder as `/play/$gameId/map`; Phase 4 replaces that placeholder with the real map.
8. Add `play.$gameId.map.tsx` as a route-level placeholder that fits the new layout and states that the map view area exists without implementing SVG map rendering yet.
...
- `/play/$gameId` opens the Star Map page inside the new game layout.
- `/play/$gameId/map` opens the same Star Map area.
...
- Phase 2 can ship before Phase 1 because it does not call `worldMaps`.
- The current action flow behaves the same as before from the player's perspective.
- `pnpm --filter frontend checks` passes.

### Phase 3: Game Creation Settings And Deterministic World Generation

Goal: collect world generation settings during game creation, deterministically generate the System and movement graph from those settings, and save the generated world when the game is created.

Files to create or update:

- `backend/src/lib/Range.ts`
- `backend/src/lib/mulberry32prng.ts`
- `backend/src/lib/world-maps/mapGenerationSettings.ts`
- `backend/src/lib/world-maps/generateWorldMap.ts`
- `backend/src/lib/world-maps/generateWorldMap.test.ts`
- `backend/src/lib/world-maps/worldMapCoordinates.ts`
...
2. Define `MapGenerationSettings` in `backend/src/lib/worldMaps/mapGenerationSettings.ts`, matching the shape in this document exactly.
3. Add Zod validation for each range: integer-like settings use integer min/max, `planetDensityOfSystem` accepts decimal values between 0 and 1, all ranges require `min <= max`, and `seed` is optional at the router boundary.
...
5. Update the `games.create` router input to require `mapGenerationSettings` alongside `name`, `nbSeats` and `tickIntervalSeconds`.
6. Normalize a missing seed in `GamesController.create` using the unix timestamp before generation and persistence. The value persisted in `game_maps.generation_settings.seed` must always be numeric.
7. Inject `WorldMapsRepository` into `GamesController`.
8. Implement `generateWorldMap` as a pure function that accepts normalized `MapGenerationSettings` and returns the full payload expected by `WorldMapsRepository.createSystem`.
...
11. Roll the fixed generation values from the configured ranges: planet density, number of Planets, number of Asteroid belts. Moons per Planet and Asteroids per Sector will be rolled per Planet/Sector to allow variance, not once per world generation.
...
21. Add coordinate formatting helpers in `worldMapCoordinates.ts` for `02`, `02:11` and `02:11:05`; use these in repository DTOs and tests.
22. Save game creation and world-map creation atomically. Extend `GamesRepository.create` with a repository-level transaction hook that receives the created game and transaction handle; `GamesController.create` passes a hook that calls `WorldMapsRepository.createSystem(..., tx)` before the game creation transaction commits.
23. Keep `GamesController.create` as the orchestrator that validates input, normalizes seed/settings, calls `generateWorldMap`, and requests persistence.
24. Wire the new `WorldMapsRepository` dependency through `createApi.ts`, `createApi.stub.ts` and `entry.api.ts`.
25. Update `games.router.test.ts` fixtures so every game creation passes deterministic map settings.
26. Add a router test proving `games.create` persists a map that is readable through `worldMaps.getSystem`.
...
1. Add a "World generation" section to `frontend/src/routes/games.new.tsx`.
...
6. Submit `mapGenerationSettings` through the existing `games.create` mutation.
7. Keep the successful navigation to `/games/$gameId`.

Definition of done:

- Creating a game stores its deterministic world map in the Phase 1 tables.
- A saved map can be read through `worldMaps.getSystem` immediately after game creation.
- The generation seed is persisted in `game_maps.generation_settings`.
- `pnpm -r checks` pass.

### Phase 4: Star Map View

Goal: replace the Phase 2 Star Map placeholder with the actual SVG + `d3-zoom` map view using persisted System data from the Phase 1 API and generated data from Phase 3.
...
- `frontend/src/routes/play.$gameId.index.tsx`
- `frontend/src/routes/play.$gameId.map.tsx`
- `frontend/src/components/StarMapView.tsx`
- `frontend/src/components/StarMapSvg.tsx`
- `frontend/src/components/StarMapControls.tsx`
- `frontend/src/components/StarMapSelectionPanel.tsx`
- `frontend/src/components/StarMapCoordinateInput.tsx`
- `frontend/src/lib/starMapGeometry.ts`
- `frontend/src/lib/starMapCoordinates.ts`
- `frontend/src/lib/useStarMapZoom.ts`
- `frontend/src/assets/star-map/*`
...
2. Query `backendApiClient.worldMaps.getSystem` from `play.$gameId.map.tsx`.
3. Make `play.$gameId.index.tsx` render the same Star Map view as `/play/$gameId/map` so both documented routes show the map.
4. Add runtime visual assets for one Planet, one Moon, one Asteroid, one star, and a few starry background variants under `frontend/src/assets/star-map/`. Do not load files from `.github/images` at runtime.
5. Implement `StarMapView` as the page-level composition: loading state, error state, coordinate controls, central map, and selection panel.
6. Implement `StarMapSvg` as the main SVG renderer. Render the star at the center, Orbits as concentric bands and Sectors as annular sector paths.
7. Implement geometry in `starMapGeometry.ts`: Sector 1 starts at 0 degrees at 12 o'clock, later Sectors proceed clockwise, and every Sector gets a stable start/end angle based on `sector_number` and the number of Sectors in that Orbit.
...
11. Implement `StarMapCoordinateInput` accepting `Orbit`, `Orbit:Sector` and `Orbit:Sector:Body` formats. Submitting an Orbit zooms to the Orbit, submitting a Sector zooms to the Sector, and submitting a Body zooms to the Body.
12. Implement coordinate parsing in `frontend/src/lib/starMapCoordinates.ts`; keep it frontend-specific and aligned with backend output formatting.
13. Allow selecting Sectors and Bodies. Selection updates `StarMapSelectionPanel` with coordinates, name, type, and movement-neighbor summary.
...
16. Keep the star map inside the Phase 2 game layout with the left nav and top info bar visible.
...
18. Confirm that pan/zoom state changes do not repeatedly refetch `worldMaps.getSystem`.
...
- `/play/$gameId` and `/play/$gameId/map` render a non-empty star map for a game created after Phase 3.
...
- The Star Map consumes the persisted world model rather than hardcoded sample data.
